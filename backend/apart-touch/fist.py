...
# 모델 로드
model = tf.keras.models.load_model('pandas/fist_open_wrong_model.h5')
...
with mp_hands.Hands(
  ...
  while cap.isOpened():
    ...
    # 반전된 이미지에 텍스트 추가
    cv2.putText(flipped_image, f"Prediction: {final_prediction}", (10, 30), cv2.FONT_HERSHEY_SIMPLEX, 1, (0, 255, 0), 2, cv2.LINE_AA)
    cv2.putText(flipped_image, f"Count: {count}", (10, 70), cv2.FONT_HERSHEY_SIMPLEX, 1, (0, 255, 0), 2, cv2.LINE_AA)
    ...
    if count >= 30:
      ...
      cv2.putText(flipped_image, text, (text_x, text_y), cv2.FONT_HERSHEY_SIMPLEX, font_scale, (0, 0, 255), font_thickness, cv2.LINE_AA)
